...
export const useProgram = () => {
  ...
  const [userProgram, setUserProgram] = useState<Program<Users> | undefined>();
  ...
  useEffect(() => {
    if (usersId && programProvider) {
      const program = new anchor.Program(usersIdl, usersId, programProvider);
      setUserProgram(program);
    }
  }, [usersId, programProvider]);
